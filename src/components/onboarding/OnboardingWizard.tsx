  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      if (!projectResponse.ok || !projectPayload.project) {
        throw new Error(projectPayload.error || 'Could not create your project')
      }

      const project = projectPayload.project as { id: string }
      const completionResponse = await fetch('/api/users/me', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ onboarding_completed: true }),
      })
      if (!completionResponse.ok) throw new Error('Project created, but we could not complete onboarding')
      ...
      if (!analysisResponse.ok) {
        toast.error('Project created. You can rerun the analysis from its page.')
      }
