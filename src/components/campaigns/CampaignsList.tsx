...
export default function CampaignsList({ projectId, platforms, refreshKey = 0 }: Props) {
  ...
  const loadCampaigns = useCallback(async () => {
    try {
      const res = await fetch(`/api/campaigns?project_id=${encodeURIComponent(projectId)}`, { cache: 'no-store' })
