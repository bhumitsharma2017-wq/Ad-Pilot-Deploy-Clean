...
function isNonProductionRuntime(): boolean {
  // Vercel Preview deployments run with NODE_ENV=production, so VERCEL_ENV is
  // the reliable guard for live test URLs. Production deployments stay locked.
...
// Development/test-only safety switch. Local dev defaults to enabled for easier
// QA, Vercel Preview needs TEST_SUBSCRIPTION_MODE=true, and Vercel Production is
// always protected from this bypass.
export function isTestSubscriptionModeEnabled(): boolean {
  if (!isNonProductionRuntime()) return false
  if (isExplicitlyDisabled(process.env.TEST_SUBSCRIPTION_MODE)) return false
  if (process.env.VERCEL_ENV === 'preview') return isExplicitlyEnabled(process.env.TEST_SUBSCRIPTION_MODE)
  return process.env.NODE_ENV === 'development' || isExplicitlyEnabled(process.env.TEST_SUBSCRIPTION_MODE)
}

// Subscription testing and AI testing are intentionally separate. A preview or
// local environment may bypass billing while still using a configured AI
// provider, which is important when validating real, customer-specific output.
export function shouldUseMockAi(): boolean {
  return isTestSubscriptionModeEnabled() && !process.env.OPENAI_API_KEY?.trim()
}

// Reports can use Anthropic when configured, or OpenAI as a dependable fallback.
// Keep preview data only for test environments where neither provider is available.
export function shouldUseMockReport(): boolean {
  return isTestSubscriptionModeEnabled()
