...
export function createMockCreatives(type: string, formats: string[], project: ProjectLike = {}) {
  const company = getCompany(project)
  const offer = project.business_analysis?.products?.[0]
    || project.business_analysis?.services?.[0]
    || project.business_analysis?.features?.[0]
    || `${company}'s offering`
  const audience = project.business_analysis?.target_audience || 'the right customers'
  const primaryUsp = project.business_analysis?.usp?.[0] || 'a clearer customer outcome'

  if (type === 'copy_variations') {
    return [
      {
        angle: 'Problem-Solution',
        headline: `${offer} for ${audience}`.slice(0, 70),
        body: `${company} helps ${audience} get ${primaryUsp.toLowerCase()}. Explore the offer built around your needs.`,
        cta: 'Learn More',
      },
      {
        angle: 'Direct Benefit',
        headline: `Discover ${offer}`.slice(0, 70),
        body: `See how ${company} delivers ${primaryUsp.toLowerCase()} for ${audience}.`,
        cta: 'Get Started',
      },
    ]
  }
  ...
  if (type === 'video_script') {
    return [
      {
        duration: 15,
        hook: `Looking for ${primaryUsp.toLowerCase()}?`,
        voiceover: `Meet ${company}. See ${offer} in action and discover an option built for ${audience}.`,
        cta: 'Learn More',
        storyboard_notes: `Open with the customer problem, show ${offer} in use, then end on the outcome and CTA.`,
      },
      {
        duration: 30,
        hook: `${offer}, made for ${audience}.`,
        voiceover: `${company} focuses on ${primaryUsp.toLowerCase()}. Show the real experience, the benefit it creates, and a clear next step.`,
        cta: 'Explore the Offer',
        storyboard_notes: `Show authentic details of ${offer}, customer context, and an outcome-led closing frame.`,
      },
    ]
  }
  ...
  return selectedFormats.map((format) => ({
    format,
    concept: `Product-led campaign scene for ${company}: show ${offer} in a real context for ${audience}, with visual evidence of ${primaryUsp.toLowerCase()}.`,
    headline: `${offer}`.slice(0, 60),
    body_text: `${primaryUsp} for ${audience}.`.slice(0, 120),
    cta: 'Learn More',
    color_scheme: ['#4f46e5', '#0ea5e9', '#10b981'],
    visual_elements: [offer, `Customer context: ${audience}`, primaryUsp],
    mood: 'Polished, specific, and outcome-led',
  }))
}
