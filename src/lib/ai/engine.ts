...
function formatList(values: string[] | undefined, fallback = 'Not provided') {
  const cleanValues = (values || []).map(value => String(value).trim()).filter(Boolean)
  return cleanValues.length ? cleanValues.join(' | ') : fallback
}

/**
 * Keeps every downstream campaign prompt tied to the analyzed business rather
 * than letting the model fill gaps with a generic SaaS/marketing narrative.
 */
function getBusinessGrounding(business: BusinessAnalysis) {
  return `SOURCE OF TRUTH — use only the business facts below. Do not invent a different
industry, product, audience, offer, feature, price, result, or brand claim.
Company: ${business.company_name}
Category: ${business.business_category}
Products: ${formatList(business.products)}
Services: ${formatList(business.services)}
Features: ${formatList(business.features)}
Verified USPs: ${formatList(business.usp)}
Target audience: ${business.target_audience || 'Not provided'}
Customer pain points: ${formatList(business.pain_points)}
Known objections: ${formatList(business.objections)}

Every headline, description, keyword, creative angle, and visual concept must clearly relate to this source of truth. Prefer specific product/service language over generic phrases such as “grow faster” or “better results”. Never mention AdPilot, campaign planning, dashboards, SaaS, or marketing unless those are explicitly listed above.`
}
...
${getBusinessGrounding(business_analysis)}
...
Write natural, benefit-led copy for this exact business. Do not use placeholders, generic filler, unsupported superlatives, or any topic outside the source of truth. Before returning, verify every character limit and that every line is relevant to the business.
Return ONLY valid JSON.`
...
Use distinct creative angles rooted in the actual product, customer problem, and verified USPs. Do not output generic marketing-tool creative concepts unless the source of truth describes a marketing tool. Return ONLY valid JSON.`
...
All targeting, headlines, descriptions, and creative angles must describe this exact business. Return ONLY valid JSON.`
...
Each visual and spoken line must be recognizably about this business and its verified offer. Return ONLY valid JSON.`
...
export async function generateReport(
  ...
  const prompt = `Generate a professional ${reportType} performance marketing report for a client.

SOURCE OF TRUTH — use only the project data below. Clearly label estimates and strategic recommendations as estimates; never present invented campaign performance, spend, conversions, revenue, or client testimonials as facts.

Project Data:
${JSON.stringify(projectData, null, 2)}

Write a client-ready report with exactly these Markdown sections:
1. Executive Summary
2. Business & Audience Insights
3. Recommended Channel Strategy
4. Campaign Assets & Messaging
5. Forecast Assumptions
6. Priority Actions for the Next Period

Keep the language specific to this business, concise, credible, and useful in a client meeting. Use clear headers and bullet points.`
  ...
  if (process.env.OPENAI_API_KEY?.trim()) {
    const response = await getOpenAIClient().chat.completions.create({
      model: process.env.OPENAI_REPORT_MODEL || process.env.OPENAI_MODEL || 'gpt-4o',
      messages: [{ role: 'user', content: prompt }],
      temperature: 0.35,
      max_tokens: 4000,
    })
  ...
  throw new Error('Configure ANTHROPIC_API_KEY or OPENAI_API_KEY to generate reports')
}
...
Only recommend products, categories, and claims that appear in the source of truth. Return ONLY valid JSON.`
