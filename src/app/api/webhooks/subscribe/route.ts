...
export async function POST(request: NextRequest) {
  try {
    ...
    if ((plan !== 'pro' && plan !== 'agency') || (billing_cycle !== 'monthly' && billing_cycle !== 'yearly')) {
      return NextResponse.json({ error: 'Choose a valid paid plan and billing cycle' }, { status: 400 })
    }
    ...
    if (!planId || !process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID) {
      return NextResponse.json({
        error: 'Payments are not configured yet. Add Razorpay key and plan IDs before enabling checkout.',
      }, { status: 503 })
    }
