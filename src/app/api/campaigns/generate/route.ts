...
import { getServerEffectivePlan, shouldUseMockAi } from '@/lib/subscription/test-mode'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (typeof project_id !== 'string') return NextResponse.json({ error: 'Project ID is required' }, { status: 400 })
    const { project, permission } = await getProjectAccess(adminClient, user.id, project_id)

    if (!project) return NextResponse.json({ error: 'Project not found' }, { status: 404 })
    if (!canEditProject(permission)) return NextResponse.json({ error: 'Viewer access cannot generate campaigns' }, { status: 403 })
    ...
    const input: GenerateCampaignInput = {
      ...
      competitor_analysis: project.competitor_analysis || undefined,
      campaign_strategy: project.campaign_strategy || undefined,
    ...
    for (const currentPlatform of platformsToGenerate) {
      ...
      // No configured AI provider means this can only return a labelled preview.
      // Test subscription access itself does not force this fallback.
      const generated = useMockAi
    ...
    await adminClient.from('usage_logs').insert({
      ...
      tokens_used: useMockAi ? 0 : 4000 * platformsToGenerate.length,
    })

    return NextResponse.json({
      success: true,
      campaigns: results,
      preview_only: useMockAi,
      message: useMockAi ? 'Preview campaigns shown. Add OPENAI_API_KEY for real AI output.' : undefined,
    })
  } catch (error) {
