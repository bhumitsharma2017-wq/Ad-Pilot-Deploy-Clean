...
import { shouldUseMockAi } from '@/lib/subscription/test-mode'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!canEditProject(permission)) return NextResponse.json({ error: 'Viewer access cannot run analysis' }, { status: 403 })
    ...
    if (shouldUseMockAi()) {
      ...
      await adminClient.from('usage_logs').insert({
        user_id: user.id,
        action: 'ai_analysis',
        metadata: { project_id, regenerate: regenerate || 'full', test_mode: true, ai_fallback: true },
        tokens_used: 0,
      })
