...
export async function POST(request: NextRequest) {
  try {
    ...
    // Test mode uses sample creative assets so Preview deployments can validate
    // Creative Studio without requiring OpenAI credentials.
    if (isTestSubscriptionModeEnabled()) {
      creatives = createMockCreatives(type, formats)
...
Return ONLY valid JSON array.`
    ...
    if (creatives.length > 0) {
      const { error: assetError } = await adminClient.from('creative_assets').insert(
