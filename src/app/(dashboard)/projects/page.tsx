...
import type { BusinessGoal, Platform, Project, Subscription } from '@/types'
...
const PLATFORMS: { value: Platform; label: string; color: string }[] = [
  ...
  { value: 'shopping', label: 'Google Shopping', color: 'bg-emerald-50 border-emerald-200 text-emerald-700' },
  { value: 'demand_gen', label: 'Demand Gen', color: 'bg-violet-50 border-violet-200 text-violet-700' },
  { value: 'performance_max', label: 'Performance Max', color: 'bg-amber-50 border-amber-200 text-amber-700' },
]
...
export default function ProjectsPage() {
  ...
  const previewPlan = resolvePreviewPlan(getClientPlanPreviewSelection())
  const projectLimit = getProjectLimitForPlan(subscription?.plan)

  useEffect(() => {
    const loadData = async () => {
      try {
        const [projectsResponse, userResponse] = await Promise.all([
          fetch('/api/projects?scope=accessible', { cache: 'no-store' }),
          fetch('/api/users/me', { cache: 'no-store' }),
        ])
        const [projectsData, userData] = await Promise.all([projectsResponse.json(), userResponse.json()])
        if (!projectsResponse.ok) throw new Error(projectsData.error || 'Failed to load projects')
        if (!userResponse.ok) throw new Error(userData.error || 'Failed to load account')
    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-0.5">
            ...
            {projectLimit !== null && ` - ${projects.length}/${projectLimit} free slots used`}
          </p>
          {previewPlan && (
            <p className="text-xs text-amber-700 mt-1">
              Preview mode is active for the {previewPlan} plan.
            </p>
          )}
        </div>
        <button
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2, 3].map((item) => (
            <div key={item} className="h-20 bg-white rounded-xl border border-gray-100 animate-pulse" />
          ))}
        </div>
      ) : projects.length === 0 ? (
