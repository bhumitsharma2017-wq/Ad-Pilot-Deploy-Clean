...
export default function ProjectDetailPage() {
  ...
  const loadProject = useCallback(async () => {
    ...
    setProject(response.ok ? (payload.data || null) : null)
    setLoading(false)
  }, [projectId])
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      const response = await fetch(`/api/projects/${encodeURIComponent(projectId)}`, { cache: 'no-store' })
      const payload = await response.json().catch(() => ({}))
      const data = response.ok ? payload.data as Project | undefined : undefined
  ...
  const handleGenerateCampaigns = async () => {
    ...
    try {
      ...
      toast.success(data.preview_only
        ? `${data.campaigns?.length || 0} preview campaign${data.campaigns?.length === 1 ? '' : 's'} created — add OPENAI_API_KEY for real AI output.`
        : `${data.campaigns?.length || 0} campaign${data.campaigns?.length === 1 ? '' : 's'} generated!`)
      setActiveTab('campaigns')
      setCampaignRefreshKey((current) => current + 1)
      await loadProject()
    } catch (error: unknown) {
      toast.error(error instanceof Error ? error.message : 'Failed to generate campaigns')
    } finally {
  ...
  const handleRunAnalysis = async () => {
    ...
    try {
      const res = await fetch('/api/ai/analyze', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ project_id: projectId, regenerate: 'full' }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Analysis failed')
      toast.success(data.test_mode ? 'Preview analysis completed — add OPENAI_API_KEY for real business analysis.' : 'Analysis completed!')
      await loadProject()
    } catch (error: unknown) {
        ...
        <div className="flex items-center gap-2">
          ...
          {isCompleted && (
            <button
              ...
              disabled={reanalyzing || regenerating}
              className="flex items-center gap-2 px-4 py-2 border border-gray-200 bg-white text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-50 transition-colors disabled:opacity-60"
            >
              {reanalyzing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Brain className="w-4 h-4" />}
              Re-run Analysis
            </button>
          )}
          {!isCompleted && (
            ...
            >
              {reanalyzing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Brain className="w-4 h-4" />}
              {project.status === 'error' ? 'Retry Analysis' : 'Run Analysis'}
            </button>
          )}
        </div>
      </div>
      ...
      {isCompleted && (
        <>
          ...
          <div className="animate-fade-in">
            ...
            {activeTab === 'campaigns' && (
              <CampaignsList projectId={projectId} platforms={project.platforms || []} refreshKey={campaignRefreshKey} />
            )}
