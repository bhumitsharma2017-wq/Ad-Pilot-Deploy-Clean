...
export default function SubscriptionPage() {
  ...
  const handleCancel = async () => {
    if (!confirm('Cancel your plan at the end of the current billing period? You will keep access until then.')) return
    setCancelling(true)
    try {
      ...
      setSubscription(data.subscription)
      toast.success(data.message || 'Your subscription will cancel at period end.')
      router.refresh()
    } catch (error: unknown) {
  ...
  return (
    <div className="max-w-5xl mx-auto animate-fade-in space-y-8">
      ...
      ) : effectiveSubscription && effectiveSubscription.plan !== 'free' ? (
        <div className="flex flex-col items-center gap-3 p-4 bg-gray-50 rounded-xl text-sm text-gray-500 text-center">
          <span>
