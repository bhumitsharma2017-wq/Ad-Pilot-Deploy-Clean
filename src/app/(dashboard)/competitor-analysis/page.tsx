...
export default function CompetitorAnalysisPage() {
  ...
  useEffect(() => {
    const loadProjects = async () => {
      try {
        const response = await fetch('/api/projects?scope=accessible&status=completed', { cache: 'no-store' })
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      // Refetch through the protected project endpoint so shared workspaces
      // receive the same result as the owner.
      const projectResponse = await fetch(`/api/projects/${encodeURIComponent(selectedProject)}`, { cache: 'no-store' })
      const projectPayload = await projectResponse.json()
      if (projectResponse.ok && projectPayload.data?.competitor_analysis) {
        setAnalysis(projectPayload.data.competitor_analysis)
      }
