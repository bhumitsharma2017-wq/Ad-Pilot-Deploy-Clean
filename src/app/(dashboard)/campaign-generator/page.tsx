...
export default function CampaignGeneratorPage() {
  ...
  const [generatedCampaignCount, setGeneratedCampaignCount] = useState(0)
  ...
  const handleGenerate = async () => {
    ...
    for (const platform of selectedPlatforms) {
      ...
      try {
        ...
        const data = await res.json()
        if (!res.ok) throw new Error(data.error || `${platform} generation failed`)
        setPreviewOnly(Boolean(data.preview_only))
        setGeneratedCampaignCount(prev => prev + (data.campaigns?.length || 0))
        ...
        toast.error(error instanceof Error ? error.message : `Failed to generate ${platform} campaign`)
      }
    }
    setGenerating(false)
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
      ...
      {generated.length > 0 && !generating && (
        <div className="bg-green-50 border border-green-200 rounded-2xl p-5 flex flex-col md:flex-row md:items-center gap-4">
          <CheckCircle className="w-6 h-6 text-green-500 flex-shrink-0" />
          <div className="flex-1">
            <div className="font-semibold text-green-900">
              {generatedCampaignCount || generated.length} campaign{(generatedCampaignCount || generated.length) !== 1 ? 's' : ''} {previewOnly ? 'previewed' : 'generated successfully'}!
            </div>
            <div className="text-sm text-green-700 mt-0.5">
              {previewOnly
                ? 'These are preview-only templates because no AI provider key is configured. Add OPENAI_API_KEY, run analysis again, then generate real campaigns.'
                : 'Campaigns are saved inside the selected project under the Campaigns tab.'}
            </div>
          </div>
          <Link
            href={`/projects/${selectedProject}?tab=campaigns`}
            className="inline-flex items-center justify-center gap-2 px-4 py-2 bg-green-600 text-white rounded-xl text-sm font-medium hover:bg-green-700 transition-colors"
          >
            Open Campaigns
