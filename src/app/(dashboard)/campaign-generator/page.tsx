...
export default function CampaignGeneratorPage() {
  ...
  useEffect(() => {
    const loadProjects = async () => {
      try {
        const res = await fetch('/api/projects?status=completed', { cache: 'no-store' })
        ...
        <div className="bg-green-50 border border-green-200 rounded-2xl p-5 flex flex-col md:flex-row md:items-center gap-4">
          ...
          <div className="flex-1">
            <div className="font-semibold text-green-900">
              {generatedCampaignCount || generated.length} campaign{(generatedCampaignCount || generated.length) !== 1 ? 's' : ''} generated successfully!
            </div>
