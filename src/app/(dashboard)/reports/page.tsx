...
export default function ReportsPage() {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      toast.success(data.preview_only ? 'Preview report generated' : 'Report generated!')
      await loadData()
    } catch (error: unknown) {
    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in">
      ...
      {previewOnly && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          The latest report is a preview because no AI provider key is configured. Add <code className="font-medium">OPENAI_API_KEY</code> or <code className="font-medium">ANTHROPIC_API_KEY</code> for real client-ready reports.
        </div>
      )}
          ...
          <div>
            ...
            {!loading && projects.length === 0 && (
              <p className="text-xs text-amber-600 mt-1.5">
                ...
                and run analysis first, then come back to generate reports.
              </p>
            )}
            {selectedProject && !canGenerateForSelectedProject && (
              <p className="text-xs text-amber-600 mt-1.5">
                Reports require Pro or Agency for this workspace. Select a paid shared project or upgrade this workspace.
              </p>
            )}
          </div>
        </div>
