...
import { Crown, Eye, Loader2, Mail, Shield, Trash2, UserPlus, Users } from 'lucide-react'
import toast from 'react-hot-toast'
import { Badge, Card, EmptyState } from '@/components/ui'
import { createClient } from '@/lib/supabase/client'
import { getClientPlanPreviewSelection, getEffectivePlan, resolvePreviewPlan } from '@/lib/subscription/preview'
...
export default function TeamPage() {
  ...
  const loadTeam = useCallback(async () => {
    ...
    const [{ data: subscription }, { data: team }, userResponse] = await Promise.all([
    ...
    const previewSelection = getClientPlanPreviewSelection()
    setIsAgency((userData.subscription?.plan || getEffectivePlan(subscription?.plan, previewSelection)) === 'agency')
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-gray-500 mt-0.5">
          {members.length} member{members.length !== 1 ? 's' : ''} - Agency plan
        </p>
        {previewPlan && (
      ...
      <Card padding="md">
        ...
        <div className="flex flex-col md:flex-row gap-3">
          ...
          <button
            ...
            disabled={inviting}
            className="flex items-center justify-center gap-2 px-4 py-2.5 bg-brand-600 text-white rounded-xl text-sm font-medium hover:bg-brand-700 disabled:opacity-60 transition-colors whitespace-nowrap"
          >
        ...
        <div className="flex flex-wrap gap-4 mt-3">
          {Object.entries(ROLE_CONFIG).map(([key, config]) => (
            <div key={key} className="flex items-center gap-1.5 text-xs text-gray-500">
              ...
              {key === 'member' && ' Can edit and generate'}
      ...
      {loading ? (
        <div className="space-y-2">
          {[1, 2].map((item) => (
            <div key={item} className="h-16 bg-white rounded-xl animate-pulse border" />
          ))}
        </div>
      ) : members.length === 0 ? (
        <Card>
            ...
            return (
              <Card key={member.id} padding="sm" className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <select
                    value={member.role}
                    onChange={(event) => updateRole(member.member_id, event.target.value)}
