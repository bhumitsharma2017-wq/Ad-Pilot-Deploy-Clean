...
OPENAI_MODEL=gpt-4o
# Image generation for Creative Studio (defaults to gpt-image-1)
...
RAZORPAY_AGENCY_MONTHLY_PLAN_ID=plan_...
RAZORPAY_AGENCY_YEARLY_PLAN_ID=plan_...

# Optional: Website crawling (better analysis)
FIRECRAWL_API_KEY=fc-...
```

`TEST_SUBSCRIPTION_MODE` only unlocks paid-plan screens in a local or preview deployment. It does not provide AI generation. Without `OPENAI_API_KEY`, the app visibly returns business-aware preview templates instead of presenting them as real analysis, ad copy, or image assets. Reports use `ANTHROPIC_API_KEY` when available and automatically fall back to `OPENAI_API_KEY`.
...
3. Run the entire contents of `supabase/schema.sql`
   - If your database was created before the Creative Studio persistence update, also run `supabase/production-upgrade.sql`.
